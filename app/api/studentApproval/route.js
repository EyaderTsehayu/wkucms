...
export const GET = async () => {
  ...
  try {
    ...
    const requests = collegeName && departmentName
      ? await StudentClearnceReq.find({ status: privilage, collegeName: collegeName, departmentName: departmentName })
      : collegeName
        ? await StudentClearnceReq.find({ status: privilage, collegeName: collegeName })
        : await StudentClearnceReq.find({ status: privilage });
